  ...
  return {
    ...
    actions: {
      list: {
        ...
        handle: (env) => {
          return k8.listJobs(env.data.namespace)
            .then((result) => {
              const list = _.map(result.items, (item) => item.metadata.name)
              return { data: list }
            })
        }
      },
      schedule: {
        ...
        url: '',
        handle: (env) => {
          const type = types[ env.headers[ 'content-type' ] ]
          let job
        ...
        handle: (env) => {
          return k8.getJobStatus(env.data.namespace, env.data.jobName)
            .then(
              ...
              () => { return { status: 404, data: `job ${env.data.jobName} not found in namespace ${env.data.namespace}` } }
            )
        }
      },
      stop: {

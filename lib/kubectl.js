...
function apply (context, input) {
  const res = ChildProcess.spawnSync('kubectl', [`--context=${context}`, 'apply', '-f', '-', '-ojson'], { input })
...
function del (context, namespace, type, name) {
  const res = ChildProcess.spawnSync('kubectl', [`--context=${context}`, `--namespace=${namespace}`, 'delete', type, name])
...
function version (context) {
  ...
  let res = ChildProcess.spawnSync('kubectl')
  if (res.error) {
    throw new Error('kubectl command not found')
  }
  res = ChildProcess.spawnSync('kubectl', ['config', 'get-contexts', context])
  if (res.status !== 0) {
    throw new Error(`context '${context}' not found`)
  }
  res = ChildProcess.spawnSync('kubectl', [`--context=${context}`, 'version', '-ojson'])
  if (res.error) {
    throw res.error
  }
